...
st.title("Estimation des erreurs de dose")
st.markdown("Importez vos fichiers RT-PLAN pour calculer les erreurs cumulées.")
...
if uploaded_files:
    ...
    # Affichage du nom et de l'ID proprement en haut
    if len(raw_data) > 0:
        col1, col2 = st.columns(2)
        col1.info(f"**Patient :** {raw_data[0]['Patient']}")
        col2.info(f"**ID :** {raw_data[0]['ID']}")

    # 2. Logique de calcul (Cumul et Séances max)
    ...
    for index, item in enumerate(raw_data):
        ...
        # Ajout de la ligne structurée
        final_table_data.append({
            "Date": item['Date'],
            "Machine": item['Machine'],
            "Dose / Fraction": f"{item['Dose (Gy)']:.2f}",
            "uLCT (%)": f"{item['uLCT (%)']:.2f}",
